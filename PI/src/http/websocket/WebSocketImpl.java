...
import http.websocket.drafts.Draft.CloseHandshakeType;
import http.websocket.drafts.Draft.HandshakeState;
import http.websocket.drafts.Draft_10;
import http.websocket.drafts.Draft_17;
import http.websocket.drafts.Draft_75;
import http.websocket.drafts.Draft_76;
import http.websocket.exceptions.IncompleteHandshakeException;
import http.websocket.exceptions.InvalidDataException;
import http.websocket.exceptions.InvalidHandshakeException;
import http.websocket.exceptions.WebsocketNotConnectedException;
import http.websocket.framing.CloseFrame;
import http.websocket.framing.CloseFrameBuilder;
import http.websocket.framing.Framedata;
import http.websocket.framing.Framedata.Opcode;
import http.websocket.handshake.ClientHandshake;
import http.websocket.handshake.ClientHandshakeBuilder;
import http.websocket.handshake.Handshakedata;
import http.websocket.handshake.ServerHandshake;
import http.websocket.handshake.ServerHandshakeBuilder;
import http.websocket.server.WebSocketServer.WebSocketWorker;
import http.websocket.util.Charsetfunctions;
...
public class WebSocketImpl implements WebSocket {
    ...
    public void decode(ByteBuffer socketBuffer) {
        ...
        if (DEBUG)
            System.out.println("process(" + socketBuffer.remaining() + "): {" + (socketBuffer.remaining() > 1000
                    ? "too big to display"
                ...
                if (curop == Opcode.CLOSING) {
                    ...
                    if (curop == Opcode.PING) {
                        ...
                        if (curop == Opcode.PONG) {
                            ...
                            if (!fin || curop == Opcode.CONTINUOUS) {
        ...
        if (getReadyState() == READYSTATE.NOT_YET_CONNECTED) {
            ...
            if (flushandclosestate) {
                ...
                if (draft.getCloseHandshakeType() == CloseHandshakeType.NONE) {
                    ...
                    if (draft.getCloseHandshakeType() == CloseHandshakeType.ONEWAY) {
                        if (role == Role.SERVER)
    ...
    private void write(ByteBuffer buf) {
        if (DEBUG)
            System.out.println("write(" + buf.remaining() + "): {" + (buf.remaining() > 1000
                    ? "too big to display" : new String(buf.array())) + "}");
