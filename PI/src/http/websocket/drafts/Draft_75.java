...
import http.websocket.exceptions.InvalidFrameException;
import http.websocket.exceptions.InvalidHandshakeException;
import http.websocket.exceptions.LimitExedeedException;
import http.websocket.exceptions.NotSendableException;
import http.websocket.framing.CloseFrame;
import http.websocket.framing.FrameBuilder;
import http.websocket.framing.Framedata;
import http.websocket.framing.Framedata.Opcode;
import http.websocket.framing.FramedataImpl1;
import http.websocket.handshake.ClientHandshake;
import http.websocket.handshake.ClientHandshakeBuilder;
import http.websocket.handshake.HandshakeBuilder;
...
public class Draft_75 extends Draft {
    ...
    public HandshakeState acceptHandshakeAsClient(ClientHandshake request, ServerHandshake response) {
        return request.getFieldValue("WebSocket-Origin").equals(response.getFieldValue("Origin")) && basicAccept(response)
                ? HandshakeState.MATCHED : HandshakeState.NOT_MATCHED;
    }
    ...
    protected List<Framedata> translateRegularFrame(ByteBuffer buffer) throws InvalidDataException {
        ...
        while (buffer.hasRemaining()) {
            ...
            if (newestByte == START_OF_FRAME) { // Beginning of Frame
                ...
                if (newestByte == END_OF_FRAME) { // End of Frame
                    if (!readingState)
                    ...
                    if (readingState) { // Regular frame data, add to current frame buffer //TODO This code is very expensive and slow
