...
/**
 * This class provides native access to serial ports and devices without
 * requiring external libraries or tools.
 *
...
public final class SerialPort {
     ...
     * <p>
     * The serial ports can be accessed by iterating through each of the
     * SerialPort objects in this array.
     * <p>
     * Note that the {@link #openPort()} method must be called before any
     * attempts to read from or write to the port. Likewise,
     * {@link #closePort()} should be called when you are finished accessing the
     * port.
     * <p>
     * All serial port parameters or timeouts can be changed at any time after
    ...
    /**
     * Allocates a {@link SerialPort} object corresponding to the user-specified
     * port descriptor.
     * <p>
     * On Windows machines, this descriptor should be in the form of
     * "COM[*]".<br>
     ...
     * the port has been opened.
     * <p>
     * Note that calling this method on an already opened port will simply
     ...
     * <p>
     * Note that calling this method on an already closed port will simply
     * return a value of true.
     *
    ...
    /**
     * Returns whether the port is currently open and available for
     * communication.
     *
     ...
     * {@link #readBytes} were to be called immediately after this method
     * returns.
     *
    ...
    /**
     * Reads up to <i>bytesToRead</i> raw data bytes from the serial port and
     * stores them in the buffer.
     * <p>
     * The length of the byte buffer must be greater than or equal to the value
     * passed in for <i>bytesToRead</i>
     * <p>
     * If no timeouts were specified or the read timeout was set to 0, this call
     * will block until <i>bytesToRead</i> bytes of data have been successfully
     * read from the serial port. Otherwise, this method will return after
     * <i>bytesToRead</i> bytes of data have been read or the number of
     * milliseconds specified by the read timeout have elapsed, whichever comes
     * first, regardless of the availability of more data.
     *
     ...
     * @return The number of bytes successfully read, or -1 if there was an
     * error reading from the port.
     */
    public final int readBytes(byte[] buffer, long bytesToRead) {
        return readBytes(portHandle, buffer, bytesToRead);
    }

    /**
     * Writes up to <i>bytesToWrite</i> raw data bytes from the buffer parameter
     * to the serial port.
     * <p>
     * The length of the byte buffer must be greater than or equal to the value
     * passed in for <i>bytesToWrite</i>
     * <p>
     * If no timeouts were specified or the write timeout was set to 0, this
     * call will block until <i>bytesToWrite</i> bytes of data have been
     * successfully written the serial port. Otherwise, this method will return
     * after <i>bytesToWrite</i> bytes of data have been written or the number
     * of milliseconds specified by the write timeout have elapsed, whichever
     * comes first, regardless of the availability of more data.
     *
     * @param buffer The buffer containing the raw data to write to the serial
     * port.
    ...
    public final int writeBytes(byte[] buffer, long bytesToWrite) {
        return writeBytes(portHandle, buffer, bytesToWrite);
    }
     ...
     * <p>
     * Calling this function enables event-based serial port callbacks to be
     * used instead of, or in addition to, direct serial port read/write calls
     * or the {@link java.io.InputStream}/{@link java.io.OutputStream}
     * interface.
     * <p>
     * The parameter passed into this method must be an implementation of either
     * the {@link SerialPortDataListener} or the
     * {@link SerialPortPacketListener}. The {@link SerialPortPacketListener}
     * interface <b>must</b> be used if you plan to use event-based reading of
     * <i>full</i> data packets over the serial port. Otherwise, the simpler
     * {@link SerialPortDataListener} may be used.
     * <p>
     * Only one listener can be registered at a time; however, that listener can
     * be used to detect multiple types of serial port events. Refer to
     * {@link SerialPortDataListener} and {@link SerialPortPacketListener} for
     * more information.
     *
     * @param listener A {@link SerialPortDataListener} or
     * {@link SerialPortPacketListener}implementation to be used for event-based
    ...
    /**
     * Removes the associated {@link SerialPortDataListener} from the serial
     * port interface.
     */
    ...
    /**
     * Returns an {@link java.io.InputStream} object associated with this serial
     * port.
     * <p>
     * Allows for easier read access of the underlying data stream and abstracts
     * away many low-level read details.
     * <p>
     * Make sure to call the {@link java.io.InputStream#close()} method when you
     * are done using this stream.
     *
     * @return An {@link java.io.InputStream} object associated with this serial
     * port.
     * @see java.io.InputStream
     */
    ...
    /**
     * Returns an {@link java.io.OutputStream} object associated with this
     * serial port.
     * <p>
     * Allows for easier write access to the underlying data stream and
     * abstracts away many low-level writing details.
     * <p>
     * Make sure to call the {@link java.io.OutputStream#close()} method when
     * you are done using this stream.
     *
     * @return An {@link java.io.OutputStream} object associated with this
     * serial port.
     * @see java.io.OutputStream
     */
     ...
     * <p>
     * The baud rate can be any arbitrary value specified by the user. The
     * default value is 9600 baud. The data bits parameter specifies how many
     * data bits to use per word. The default is 8, but any values from 5 to 8
     * are acceptable.
     * <p>
     * The default number of stop bits is 1, but 2 bits can also be used or even
     * 1.5 on Windows machines. Please use the built-in constants for this
     * parameter
     * ({@link #ONE_STOP_BIT}, {@link #ONE_POINT_FIVE_STOP_BITS}, {@link #TWO_STOP_BITS}).
     * <p>
     * The parity parameter specifies how error detection is carried out. Again,
     * the built-in constants should be used. Acceptable values are
     * {@link #NO_PARITY}, {@link #EVEN_PARITY}, {@link #ODD_PARITY}, {@link #MARK_PARITY},
     * and {@link #SPACE_PARITY}.
     *
     ...
     * <p>
     * <i>Note that write timeouts are only available on Windows-based systems.
     * There is no functionality to set a write timeout on other operating
     * systems.</i>
     * <p>
     * The built-in timeout mode constants should be used ({@link #TIMEOUT_NONBLOCKING}, {@link #TIMEOUT_READ_SEMI_BLOCKING},
     * {@link #TIMEOUT_WRITE_SEMI_BLOCKING}, {@link #TIMEOUT_READ_BLOCKING}, {@link #TIMEOUT_WRITE_BLOCKING}, {@link #TIMEOUT_SCANNER})
     * to specify how timeouts are to be handled.
     * <p>
     * Valid modes are:
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Non-blocking:
     * {@link #TIMEOUT_NONBLOCKING}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Read Semi-blocking:
     * {@link #TIMEOUT_READ_SEMI_BLOCKING}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Read/Write Semi-blocking:
     * {@link #TIMEOUT_READ_SEMI_BLOCKING} | {@link #TIMEOUT_WRITE_SEMI_BLOCKING}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Read Semi-Blocking/Write Full-blocking:
     * {@link #TIMEOUT_READ_SEMI_BLOCKING} | {@link #TIMEOUT_WRITE_BLOCKING}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Read Full-blocking:
     * {@link #TIMEOUT_READ_BLOCKING}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Read Full-blocking/Write Semi-blocking:
     * {@link #TIMEOUT_READ_BLOCKING} | {@link #TIMEOUT_WRITE_SEMI_BLOCKING}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Read/Write Full-blocking:
     * {@link #TIMEOUT_READ_BLOCKING} | {@link #TIMEOUT_WRITE_BLOCKING}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Scanner: {@link #TIMEOUT_SCANNER}<br>
     * <p>
     * The {@link #TIMEOUT_NONBLOCKING} mode specifies that the corresponding
     * {@link #readBytes(byte[],long)} or {@link #writeBytes(byte[],long)} call
     * will return immediately with any available data.
     * <p>
     * The {@link #TIMEOUT_READ_SEMI_BLOCKING} or
     * {@link #TIMEOUT_WRITE_SEMI_BLOCKING} modes specify that the corresponding
     * calls will block until either
     * <i>newReadTimeout</i> or <i>newWriteTimeout</i> milliseconds of
     * inactivity have elapsed or at least 1 byte of data can be written or
     * read.
     * <p>
     * The {@link #TIMEOUT_READ_BLOCKING} or {@link #TIMEOUT_WRITE_BLOCKING}
     * modes specify that the corresponding call will block until either
     * <i>newReadTimeout</i> or <i>newWriteTimeout</i> milliseconds have elapsed
     * since the start of the call or the total number of requested bytes can be
     * written or returned.
     * <p>
     * The {@link #TIMEOUT_SCANNER} mode is intended for use with the Java
     * {@link java.util.Scanner} class for reading from the serial port. In this
     * mode, manually specified timeouts are ignored to ensure compatibility
     * with the Java specification.
     * <p>
     * A value of 0 for either <i>newReadTimeout</i> or <i>newWriteTimeout</i>
     * indicates that a {@link #readBytes(byte[],long)} or
     * {@link #writeBytes(byte[],long)} call should block forever until it can
     * return successfully (based upon the current timeout mode specified).
     * <p>
     * It is important to note that non-Windows operating systems only allow
     * decisecond (1/10th of a second) granularity for serial port timeouts. As
     * such, your millisecond timeout value will be rounded to the nearest
     * decisecond under Linux or Mac OS. To ensure consistent performance across
     * multiple platforms, it is advisable that you set your timeout values to
     * be multiples of 100, although this is not strictly enforced.
     *
     * @param newTimeoutMode The new timeout mode as specified above.
     * @param newReadTimeout The number of milliseconds of inactivity to
     * tolerate before returning from a {@link #readBytes(byte[],long)} call.
     * @param newWriteTimeout The number of milliseconds of inactivity to
     * tolerate before returning from a {@link #writeBytes(byte[],long)} call.
     */
     ...
     * <p>
     * The default number of stop bits per word is 1. Built-in stop-bit
     * constants should be used in this method
     * ({@link #ONE_STOP_BIT}, {@link #ONE_POINT_FIVE_STOP_BITS}, {@link #TWO_STOP_BITS}).
     * <p>
     * Note that {@link #ONE_POINT_FIVE_STOP_BITS} stop bits may not be
     ...
     * <p>
     * By default, no flow control is enabled. Built-in flow control constants
     * should be used in this method ({@link #FLOW_CONTROL_RTS_ENABLED}, {@link #FLOW_CONTROL_CTS_ENABLED}, {@link #FLOW_CONTROL_DTR_ENABLED},
     * {@link #FLOW_CONTROL_DSR_ENABLED}, {@link #FLOW_CONTROL_XONXOFF_IN_ENABLED}, {@link #FLOW_CONTROL_XONXOFF_OUT_ENABLED}),
     ...
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;RTS/CTS:
     * {@link #FLOW_CONTROL_RTS_ENABLED} | {@link #FLOW_CONTROL_CTS_ENABLED}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;DSR: {@link #FLOW_CONTROL_DSR_ENABLED}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;DTR/DSR:
     * {@link #FLOW_CONTROL_DTR_ENABLED} | {@link #FLOW_CONTROL_DSR_ENABLED}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;XOn/XOff:
     * {@link #FLOW_CONTROL_XONXOFF_IN_ENABLED} | {@link #FLOW_CONTROL_XONXOFF_OUT_ENABLED}
     * <p>
     * Note that only one valid flow control configuration can be used at any
     * time. For example, attempting to use both XOn/XOff
     * <b>and</b> RTS/CTS will most likely result in an unusable serial port.
     * <p>
     * Also note that some flow control modes are only available on certain
     * operating systems. Valid modes for each OS are:
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Windows: CTS, RTS/CTS, DSR, DTR/DSR, Xon,
     * Xoff, Xon/Xoff<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mac: RTS/CTS, Xon, Xoff, Xon/Xoff<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Linux: RTS/CTS, Xon, Xoff, Xon/Xoff
     *
     * @param newFlowControlSettings The desired type of flow control to enable
     ...
     * <p>
     * The parity parameter specifies how error detection is carried out. The
     * built-in parity constants should be used. Acceptable values are
     * {@link #NO_PARITY}, {@link #EVEN_PARITY}, {@link #ODD_PARITY}, {@link #MARK_PARITY},
     * and {@link #SPACE_PARITY}.
     *
    ...
    /**
     * Gets a descriptive string representing this serial port or the device
     * connected to it.
     * <p>
     * This description is generated by the operating system and may or may not
     * be a good representation of the actual port or device it describes.
     *
     * @return A descriptive string representing this serial port.
     */
    public final String getDescriptivePortName() {
        return portString.trim();
    }

    /**
     * Gets the operating system-defined device name corresponding to this
     * serial port.
     *
     * @return The system-defined device name of this serial port.
     */
    public final String getSystemPortName() {
        return (isWindows ? comPort.substring(comPort.lastIndexOf('\\') + 1) : comPort.substring(comPort.lastIndexOf('/') + 1));
    }
     ...
     * <p>
     * The return value should not be interpreted as an integer, but rather
     * compared with the built-in stop bit constants
     * ({@link #ONE_STOP_BIT}, {@link #ONE_POINT_FIVE_STOP_BITS}, {@link #TWO_STOP_BITS}).
     *
     ...
     * <p>
     * The return value should not be interpreted as an integer, but rather
     * compared with the built-in parity constants
     * ({@link #NO_PARITY}, {@link #EVEN_PARITY}, {@link #ODD_PARITY}, {@link #MARK_PARITY},
     * and {@link #SPACE_PARITY}).
     *
    ...
    /**
     * Gets the number of milliseconds of inactivity to tolerate before
     * returning from a {@link #readBytes(byte[],long)} call.
     * <p>
     * A value of 0 indicates that a {@link #readBytes(byte[],long)} call will
     * block forever until it has successfully read the indicated number of
     * bytes from the serial port.
     * <p>
     * Any value other than 0 indicates the number of milliseconds of inactivity
     * that will be tolerated before the {@link #readBytes(byte[],long)} call
     * will return.
     *
     * @return The number of milliseconds of inactivity to tolerate before
     * returning from a {@link #readBytes(byte[],long)} call.
     */
    ...
    /**
     * Gets the number of milliseconds of inactivity to tolerate before
     * returning from a {@link #writeBytes(byte[],long)} call.
     * <p>
     * A value of 0 indicates that a {@link #writeBytes(byte[],long)} call will
     * block forever until it has successfully written the indicated number of
     * bytes to the serial port.
     * <p>
     * Any value other than 0 indicates the number of milliseconds of inactivity
     * that will be tolerated before the {@link #writeBytes(byte[],long)} call
     * will return.
     * <p>
     * Note that write timeouts are only available on Windows operating systems.
     * This value is ignored on all other systems.
     *
     * @return The number of milliseconds of inactivity to tolerate before
     * returning from a {@link #writeBytes(byte[],long)} call.
     */
     ...
     * <p>
     * The integer result should be masked with the built-in flow control
     * constants to test if the desired setting is enabled. Valid flow control
     * configurations are:
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;None: {@link #FLOW_CONTROL_DISABLED}<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;CTS: {@link #FLOW_CONTROL_CTS_ENABLED}<br>
     ...
     * {@link #FLOW_CONTROL_XONXOFF_IN_ENABLED} | {@link #FLOW_CONTROL_XONXOFF_OUT_ENABLED}
     * <p>
     * Note that some flow control modes are only available on certain operating
     * systems. Valid modes for each OS are:
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Windows: CTS, RTS/CTS, DSR, DTR/DSR, Xon,
     * Xoff, Xon/Xoff<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mac: RTS/CTS, Xon, Xoff, Xon/Xoff<br>
    ...
    private final class SerialPortEventListener {
        ...
        public SerialPortEventListener(int packetSizeToReceive) {
            dataPacket = new byte[packetSizeToReceive];
        }
